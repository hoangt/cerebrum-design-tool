...
namespace Netron.Diagramming.Core
{
    ...
    public abstract class ControllerBase : IUndoSupport, IController
    {
        ...
        public void RemoveTool(ITool tool)
        {
            ...
            // Remove the tool from the collection even if it doesn't attach to 
            // anything (yet)
        ...
        // ------------------------------------------------------------------
        /// <summary>
        /// Replaces a tool with the same name as the specified tool, with the new tool supplied, if it exists.  Otherwise, add the new tool with its name
        /// </summary>
        /// <param name="tool">The tool.</param>
        // ------------------------------------------------------------------
        public void AddOverrideTool(ITool tool)
        {
            foreach (ITool t in this.registeredTools)
            {
                if (String.Compare(t.Name, tool.Name) == 0)
                {
                    RemoveTool(t);
                    break;
                }
