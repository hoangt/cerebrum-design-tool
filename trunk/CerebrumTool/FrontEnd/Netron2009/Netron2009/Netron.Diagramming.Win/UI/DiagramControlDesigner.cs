...
namespace Netron.Diagramming.Win
{
    ...
    internal class DiagramControlDesigner : ControlDesigner
	{
		...
		protected override void OnPaintAdornments(PaintEventArgs pe)
		{
			...
			g.DrawString("Netron Light Version " + ass.GetName().Version.ToString(),ArtPalette.DefaultFont,Brushes.DimGray,new PointF(50,130));
            #endregion

            #region Some help for the uninitiated

            g.DrawString("Control and shapes properties at run-time:", ArtPalette.DefaultBoldFont, Brushes.DimGray, 50, 300);
			g.DrawString( "The properties of the diagram control and diagram entities are accessible via the PropertyGrid, you need to connect the graph control to the PropertyGrid via the OnShowCanvasProperties event.",ArtPalette.DefaultFont, Brushes.DimGray,new Rectangle(50,320, 500,300));

            g.DrawString("Design-time properties:", ArtPalette.DefaultBoldFont, Brushes.DimGray, 50, 370);
            g.DrawString("You can customize the control inside Visual Studio by means of the PropertyGrid and the SmartTag. Note in particular the events you can handle, see the Events tab in the PropertyGrid or shift to code-view to see the API of the diagram control.", ArtPalette.DefaultFont, Brushes.DimGray, new Rectangle(50, 390, 500, 360));

            g.DrawString("Diagram library API:", ArtPalette.DefaultBoldFont, Brushes.DimGray, 50, 450);
            g.DrawString("The control is just the UI portion of the diagramming library, there is a rich document object model inside the library as well as many utilities and graph analysis functionalities. Switch to code-view and start exploring the API!", ArtPalette.DefaultFont, Brushes.DimGray, new Rectangle(50, 470, 500, 360));
            #endregion
        }
